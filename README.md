...
Mobiilissa voi tällä hetkellä luoda session, valita session arvostelu- ja tulosnäkyvyysasetukset, avata jaetun sessiolinkin, arvostella juomat sliderilla tai 5/10 kokonaisen tähden asteikolla, lisätä kommentit, tallentaa arviot, katsoa tulokset, jakaa sessiolinkin sekä hostina muokata sessiota ja juomia. Luonnissa ja muokkauksessa juomien järjestystä voi vaihtaa ylös/alas-toiminnoilla. Muokkaus tukee kuvan lisäämistä kamerasta tai kuvakirjastosta, kuvan latausta R2:een ja nimen tunnistusta Workers AI:lla. iOS-kuvakirjaston valinta käyttää system picker -virtaa ilman erillistä ennakkolupadialogia, jotta picker ei jää jumiin. Custom `Vibration`-haptics ei ole käytössä; mobiili käyttää `expo-haptics`-palautetta vain selkeissä valinnoissa, sliderin vapautuksessa sekä onnistuneissa tai epäonnistuneissa tallennustoiminnoissa, ei valikkojen tai näkymien vaihdoissa. Alareunan tab-navigaatio on poistettu; etusivun oikean yläkulman profiili-ikoni avaa Tilin suoraan. Sessionäkymässä jako ja hostin muokkaus ovat erillisinä toimintoina. Tili-näkymä aukeaa natiivina stack-siirtymänä, sisältää kielivalinnan ja tukee sähköpostiin lähetettävää kertakäyttökoodia, näyttää resend-cooldownin, linkittää tämän laitteen aiemmat arviot tiliin ja näyttää tilille talletetut arvostelusessiot public-linkkeinä. Mobiili linkittää julkisiin privacy/support/delete-account-sivuihin ja pitää valitun paikallisen kuvan tiedostonimen piilossa. Mobiilin tumma Breview-ilme pidetään linjassa webin kanssa, vaikka komponenttipohja on eri.
