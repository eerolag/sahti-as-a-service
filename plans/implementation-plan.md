...
- Web uses React, Vite, TypeScript, Tailwind, and shadcn/ui for production UI components.
- Mobile uses Expo Router, TypeScript, AniUI, and NativeWind for native iOS and Android screens.
- Shared framework-agnostic contracts and domain logic live in `src/shared`; web and mobile clients must consume those contracts instead of duplicating API shapes.
...
This slice creates the production planning baseline and completes the first Breview naming cleanup without scaffolding the Expo app or changing Cloudflare infrastructure.

Acceptance criteria:

- `plans/implementation-plan.md` and `plans/roadmap.md` exist and describe the production path.
- `README.md` tells future agents to read and update the plans before and during work.
- User-facing web copy says Breview where it previously used the old app name.
